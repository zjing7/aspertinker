...
def main3():
    ...
    #qAB.transform(list(range(nA+1, nA+nB+1)), params=[1, 2, 3, 0, 0, 0])
    qA.set_group_index([range(1, nA//2+2), range(nA//2+2, nA+1)])
    qA.write_struct('rot_x0.xyz', ftype='xyz')
    qA.transform(list(range(1, nA//2+2)), params=[0.0, 0.0, 0.0, 0, np.pi/6, 0])
    qA.write_struct('rot_x.xyz', ftype='xyz')
def main2():
    inps = sys.argv[1:3]
    qA = QMInput()
    qB = QMInput()
